...
from model import GWGRL_Multimodal, LogReg
...
def main():
    ...
    parser.add_argument('--multimodal', action='store_true', help='Use multimodal dataset.')
    parser.add_argument('--multimodal_path', type=str, default=None, help='Path to multimodal dataset.')

    parser.add_argument('--dataname', type=str, default='ele-fashion', help='Name of dataset.')
    
    parser.add_argument('--feature_type', type=str, default='t5vit', help='Type of node features (e.g., t5vit, clip).')
    ...
    #1e-3 5e-2
    parser.add_argument('--lr1', type=float, default=0.002747501509, help='Learning rate of GWGRL.')
    #1e-2 5e-5 0    1e-2   1e-5  
    ...
    parser.add_argument("--out_dim", type=int, default=1024, help='Output layer dim.')
    ...
    # 多模态
    if args.multimodal:
        graph, feat, labels, num_class, train_idx, val_idx, test_idx = load(
            name=args.dataname,
            multimodal=True,
            feature_type=args.feature_type,
            multimodal_path=args.multimodal_path
        )
        print("Loaded multimodal dataset.")
    else:
        graph, feat, labels, num_class, train_idx, val_idx, test_idx = load(args.dataname)
    ...
    for run in range(args.runs):
        ...
        model = GWGRL(in_dim=feat.shape[1], hid_dim=args.hid_dim, out_dim=args.out_dim, n_layers=args.n_layers, num_groups=args.num_groups)
        ...
        for epoch in range(args.epochs):
            ...
            # if args.multimodal:
            #    z1 = model(graph1, text_feat1, vision_feat1)
            #    z2 = model(graph2, text_feat2, vision_feat2)
            # else:
            #    z1 = model(graph1, feat1)
            #    z2 = model(graph2, feat2)
